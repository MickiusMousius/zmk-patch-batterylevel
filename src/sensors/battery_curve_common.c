 ...
 *
 * @file battery_curve_common.c
 * @brief Common implementation for translating battery voltage into state of charge.
 * @details Contains the concrete lookup table (discharge curve) for standard 1S LiPo batteries, mapping voltage ranges
 * to percentages. It implements the interpolation logic to smoothly translate raw millivolts into a reliable 0-100%
 * capacity value.
 */
...
uint8_t lipo_mv_to_pct(int32_t mv) {
    ...
    for (size_t i = 1; i < ARRAY_SIZE(lipo_curve); i++) {
        ...
        if (mv >= lower->millivolts) {
            return lower->percent + (int32_t)(upper->percent - lower->percent) * (mv - lower->millivolts) /
